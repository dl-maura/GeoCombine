...
<xsl:variable name="upperCorner">
  ...
  <xsl:variable name="geoserver_root">
    <xsl:text>https://example.com/geoserver</xsl:text>
  </xsl:variable>

  <xsl:variable name="fgdc_base">
    <xsl:text>https://example.com/your-metadata</xsl:text>
  </xsl:variable>

  <xsl:variable name="lowercase" select="'abcdefghijklmnopqrstuvwxyz-'" />
  <xsl:variable name="uppercase" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ_'" />
  <!-- double quotes within double quotes makes json parse mad so turning those into apos -->
  ...
  <xsl:template match="metadata">
    ...
    <!-- Uncomment me when you've edited your variables, can add others as relevant to your institution
    ...
    <xsl:if test="idinfo/timeperd/timeinfo">
      <xsl:choose>
        ...
        <!-- currently the schema is built so one has to have a solr_year_i setting to a non real value for cleanup but this is optional
        <xsl:otherwise>
